...
- [~] Remote: resolve the FlashPrefill V1 32K reproduction discrepancy before method selection.
  ...
  - [ ] Run the dense/`fp_v1` 20-sample-per-task RULER pilot. The first attempt stopped before input preparation because `huggingface.co` timed out and the pinned files were not cached; no generation or score was produced.
  ...
  - [ ] If the pilot justifies expansion, run the official cap of 100 samples per task.
