...
## Phase 9 — Qwen3.5-4B on CL-bench

- [x] Resolve the requested benchmark as Tencent-Hunyuan `CL-bench` for professional-domain Context Learning, not the unrelated continual-learning benchmark with the same abbreviation.
- [x] Pin `Qwen/Qwen3.5-4B` revision `851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a`, `tencent/CL-bench` revision `b28a5832a09b0d96c0cf4c22e90d7c60ede25b80`, and official evaluation code commit `16bffd1cfa05927e72ec75c835177d6e23e82172`.
- [x] Add an isolated Qwen3.5 environment and a CL-bench-only paired Full/V1 runner without changing the frozen Qwen2.5 environment or result paths.
- [~] Apply V1 only to Qwen3.5's eight `full_attention` layers; preserve all 24 Gated Delta/linear-attention layers. The 16:4 GQA, head-dim-256 math gate is implemented but still awaits the remote RTX 4090.
- [~] Implement deterministic category/sub-category-balanced 100-task native-prompt preparation, retaining source rows, exact token IDs, rubrics, hashes, and official-compatible judge inputs without truncation. The fixed remote snapshot is not yet generated.
- [x] Run local bytecode compilation, Git Bash syntax checks, and `git diff --check` for the Qwen3.5/CL-bench path (PASS on 2026-09-21); no local Torch/CUDA environment is available, so imports and numerical execution remain remote.
- [ ] Remote: run the Qwen3.5 shape/math gate, a short Full/V1 smoke test, and only then the fixed 100-task panel.
- [ ] Grade the two saved raw-output sets with the pinned official CL-bench rubric judge (`gpt-5.1`, low reasoning effort), then produce paired quality, category, length, and latency reports without rerunning generation.
...
- 2026-09-21: started the isolated Qwen3.5 continuation. The official 4B config is a 32-layer hybrid with 24 linear-attention and eight full-attention layers, 16 query heads, four KV heads, and head dimension 256. The existing Transformers 4.51.3 environment cannot load it, so the new path will clone the CUDA/Torch environment and pin Transformers 5.17.0 separately. Model/data/code download commands were issued before implementation so the remote transfer can proceed in parallel.
- 2026-09-21: implemented the isolated `exp35` setup, package-scoped imports that avoid colliding with Hugging Face's `kernels` distribution, a Qwen3.5 d=256 V1 math gate, deterministic CL-bench input preparation, paired Full/V1 timing and generation with exact-prefix resume, final-only official judge exports, and post-judge paired/subgroup reporting. Python bytecode compilation, Git Bash syntax checks, and diff checks pass locally. The local Python installation has no Torch, so imports/numerics were not run; no Qwen3.5 GPU result or CL-bench score exists yet.
- 2026-09-21: remote CL-bench preparation exposed Transformers 5.17.0's `apply_chat_template(return_dict=True)` default: the runner iterated `BatchEncoding` field names as token IDs. Explicitly request `return_dict=False` in `encode_messages`; the pinned upstream implementation then returns `out["input_ids"]`. The failure precedes input and metadata writes, so `all` can be rerun directly after pulling the fix. Local Python compilation and diff checks pass; tokenizer/model execution remains remote.
