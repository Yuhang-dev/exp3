...
- [x] Add a LongBench v2 native-32K loader using the official zero-shot prompt and exact A/B/C/D scorer, with no middle truncation and deterministic domain-balanced selection.
...
- [ ] Remote: download and hash-check both sources, then prepare and inspect the 24 fixed LongBench v2 inputs.
- [ ] Remote: run the 24-row LongBench v2 Full/V1 pilot and audit raw scores, domain coverage, prompt lengths, density/profile, and paired latency.
- [ ] Remote: run the 20-row BFCL V4 Full/V1 pilot and audit official success, trajectory divergence, context overflows, identical-prompt coverage, and paired latency before deciding whether to expand to all 200 rows.
